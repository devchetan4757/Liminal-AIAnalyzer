...
import { getNetlifySiteLogs } from '../../api/client'
...
/**
 * On-demand build-log view for a site's most recent deploy. Fetch only
 ...
 * render/ServiceLogsPanel.jsx and vercel/DeploymentLogsPanel.jsx - see
 * backend routers/netlify.py + services/integrations/netlify/sync.py
 * (site_logs) for why this surfaces the latest deploy's build-step
 * summary rather than a live runtime-log stream: Netlify's REST API has
 * no queryable runtime-log endpoint the way Render's does.
 */
export function SiteLogsPanel({ integrationId, siteId, siteName, onClose }) {
  ...
  const load = async (isPoll = false) => {
    ...
    setError('')
    try {
      const result = await getNetlifySiteLogs(integrationId, siteId, { limit: 200 })
      const incoming = result.logs || []
      incoming.forEach((l) => seenIds.current.add(l.id))
      setLogs(sortNewestFirst(incoming))
      setDeployState(result.deploy_state || null)
    } catch (err) {
      ...
      <div className="flex max-h-[85vh] w-full max-w-3xl flex-col overflow-hidden rounded-lg border border-border bg-bg-raised shadow-xl">
        <div className="flex items-center justify-between border-b border-border px-5 py-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-[11px] text-text-faint truncate max-w-[380px]">
                {siteName}{deployState ? ` · latest deploy: ${deployState}` : ''}
              </p>
            </div>
          </div>
          <div className="flex items-center gap-2">
        ...
        <div className="flex-1 overflow-y-auto bg-[#0b0e11] px-4 py-3 font-mono text-[12px] leading-relaxed">
          ...
          {!loading && !error && logs.length === 0 && (
            <p className="py-8 text-center text-white/40">No build-log lines returned for this site yet.</p>
          )}
        ...
        <div className="border-t border-border px-5 py-2 text-[10px] text-text-faint">
          {live ? `Auto-refreshing every ${POLL_MS / 1000}s while this panel is open` : 'Live updates paused'} · newest first · latest deploy's build steps only
        </div>
      </div>
    </div>
  )
