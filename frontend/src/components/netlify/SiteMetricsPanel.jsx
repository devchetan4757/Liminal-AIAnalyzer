...
import { getNetlifySitePerformance } from '../../api/client'
...
function ResponseTimeChart({ points }) {
  ...
  if (!withValues.length) {
    return (
      <div className="flex h-28 items-center justify-center rounded-lg bg-black/20 text-xs text-danger">
        Every check in this window failed — the site looks unreachable.
      </div>
    )
  }
  ...
  return (
    ...
    >
      <defs>
        <linearGradient id="netlifyRespTimeGrad" x1="0" y1="0" x2="0" y2="1">
...
// ── Status summary row (real "is this site up right now" state) ──────────
...
// Mirrors render/ServiceMetricsPanel.jsx and vercel/ProjectMetricsPanel.jsx,
// backed by the site-scoped /netlify/sites/{id}/performance endpoint
// (see routers/netlify.py).

export function SiteMetricsPanel({ integrationId, siteId, siteName, onClose }) {
  ...
  const load = async () => {
    ...
    setError('')
    try {
      const result = await getNetlifySitePerformance(integrationId, siteId, { hours })
      setData(result)
    } catch (err) {
  ...
  useEffect(() => { load() }, [integrationId, siteId, hours])
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden border-l border-border bg-bg">
      ...
      <div className="flex items-center justify-between border-b border-border px-5 py-3 flex-shrink-0">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <div className="min-w-0">
            <p className="text-sm font-semibold text-text truncate">{siteName}</p>
          </div>
        </div>
          ...
          <>
            ...
            {data.site_url && (
              <p className="text-[10px] text-text-faint break-all">
                Checking: <span className="font-mono text-text-dim">{data.site_url}</span>
              </p>
            )}
          </>
        )}
      ...
      <div className="border-t border-border px-5 py-2 text-[10px] text-text-faint flex-shrink-0">
        Live HTTP check against the site's own public URL · works regardless of plan
      </div>
    </div>
  )
}
