...
import { getVercelDeploymentLogs } from '../../api/client'
...
/**
 * On-demand, live-tailing build-log view for a single deployment. Fetch
 * only starts once the user opens this panel (not part of the dashboard's
 * passive status poll), and stops the moment it's closed. Mirrors
 * render/ServiceLogsPanel.jsx - see backend routers/vercel.py for why
 * this surfaces build output (command/stdout/stderr/fatal) rather than
 * a live runtime-log stream.
 */
export function DeploymentLogsPanel({ integrationId, deploymentId, projectName, onClose }) {
  ...
  const load = async (isPoll = false) => {
    ...
    setError('')
    try {
      const result = await getVercelDeploymentLogs(integrationId, deploymentId, { limit: 200 })
  ...
  }, [live, integrationId, deploymentId])
      ...
      <div className="flex max-h-[85vh] w-full max-w-3xl flex-col overflow-hidden rounded-lg border border-border bg-bg-raised shadow-xl">
        <div className="flex items-center justify-between border-b border-border px-5 py-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h2 className="text-sm font-semibold text-text">Build logs</h2>
              <p className="text-[11px] text-text-faint truncate max-w-[380px]">{projectName}</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <button
        ...
        <div className="flex-1 overflow-y-auto bg-[#0b0e11] px-4 py-3 font-mono text-[12px] leading-relaxed">
          ...
          {!loading && !error && logs.length === 0 && (
            <p className="py-8 text-center text-white/40">No build-log lines returned for this deployment yet.</p>
          )}
            ...
            <div className="flex flex-col">
              {logs.map((line) => (
                ...
                >
                  ...
                  {line.type && (
                    <span className={`shrink-0 w-14 uppercase text-[10px] font-bold ${LEVEL_COLOR[line.level] || LEVEL_COLOR[line.type] || 'text-white/50'}`}>
                      {line.type}
                    </span>
                  )}
        ...
        <div className="border-t border-border px-5 py-2 text-[10px] text-text-faint">
          {live ? `Auto-refreshing every ${POLL_MS / 1000}s while this panel is open` : 'Live updates paused'} · newest first · build output only
        </div>
      </div>
    </div>
  )
