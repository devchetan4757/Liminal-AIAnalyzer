...
import { getVercelProjectPerformance } from '../../api/client'
...
// chart's shape instead of silently disappearing. Mirrors
// render/ServiceMetricsPanel.jsx's ResponseTimeChart exactly.
...
function ResponseTimeChart({ points }) {
  ...
  if (!withValues.length) {
    return (
      <div className="flex h-28 items-center justify-center rounded-lg bg-black/20 text-xs text-danger">
        Every check in this window failed — the project looks unreachable.
      </div>
    )
  }
  ...
  return (
    ...
    >
      <defs>
        <linearGradient id="vercelRespTimeGrad" x1="0" y1="0" x2="0" y2="1">
...
// ── Status summary row (real "is this deployment up right now" state) ────
...
// Mirrors render/ServiceMetricsPanel.jsx, backed by the project-scoped
// /vercel/projects/{id}/performance endpoint (see routers/vercel.py).

export function ProjectMetricsPanel({ integrationId, projectId, projectName, framework, onClose }) {
  ...
  const load = async () => {
    ...
    setError('')
    try {
      const result = await getVercelProjectPerformance(integrationId, projectId, { hours })
      setData(result)
    } catch (err) {
  ...
  useEffect(() => { load() }, [integrationId, projectId, hours])
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden border-l border-border bg-bg">
      ...
      <div className="flex items-center justify-between border-b border-border px-5 py-3 flex-shrink-0">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <div className="min-w-0">
            ...
            {framework && (
              <p className="text-[10px] text-text-faint capitalize">{framework.replace(/_/g, ' ')}</p>
            )}
          </div>
        </div>
          ...
          <>
            ...
            {data.project_url && (
              <p className="text-[10px] text-text-faint break-all">
                Checking: <span className="font-mono text-text-dim">{data.project_url}</span>
              </p>
            )}
          </>
        )}
      ...
      <div className="border-t border-border px-5 py-2 text-[10px] text-text-faint flex-shrink-0">
        Live HTTP check against the project's latest deployment URL · works regardless of plan
      </div>
    </div>
  )
}
